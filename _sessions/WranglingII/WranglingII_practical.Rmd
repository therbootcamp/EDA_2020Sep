...
10. Verwende nun das `names_prefix` Argument um die Präfixe der Jahreszahlen automatisch zu entfernen. Der `character`-String der für `names_prefix` bereit gestellt wird, wird vom Beginn der Variablennamen entfernt. 
...
1. Nun nimm an, dass du die Daten als `bevoelkerung_long` im Long Format bekommen hättest und sie ins Wide Format umwandeln müsstest. Verwende die `pivot_wider()` Funktion und den Argumenten `names_from` und `values_from` die Spalten `jahr` und `einwohner` zu. Speichere das Ergebnis als `bevoelkerung_wide` ab.
...
2. Berechne wieder den Mittelwert aber verwende diesmal `summarise()` Funktion. Besteht ein Unterschied in den Resultaten dieser und der vorherigen Aufgabe?
